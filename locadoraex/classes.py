...
Session = sessionmaker(bind=engine)  # As sessões são como "contextos" onde você pode executar várias operações antes de confirmar as alterações no banco de dados. 
session = Session()

#Define a base para os modelos declarativos. Essa classe é utilizada para mapear as classes Python para as tabelas do banco de dados.
...
class Diretor(Base):
    __tablename__ = 'diretores'  #nome da tabela
    ...
    def __repr__(self):  #
        #Essa linha cria uma lista com os títulos de todos os filmes do diretor e junta esses títulos em uma única string, separando-os por vírgulas.
...
class Filme(Base):
    ...
    #chave estrangeira referenciando a tabela funcionarios. Isso indica qual diretor está responsável pelo filme
    diretor_id = Column(Integer, ForeignKey('diretores.id')) 
    #estabelece um relacionamento entre filmes e diretores
    diretor = relationship('Diretor', backref='filmes')
    #cria um relacionamento bidirecional, permitindo que você acesse os filmes de um diretor a partir do objeto Diretor
    qtdDisponivel = Column(Integer)
    funcionario_id = Column(Integer, ForeignKey('funcionarios.id'))  
    funcionario = relationship('Funcionario', backref='filmes')

   #metodo especial 
# Essa string será a representação textual do objeto quando ele for impresso.
    def __repr__(self):
...
def adicionar_diretor(nome, nacionalidade):
    #verifica se o diretor ja existe no banco de dados
    diretor = session.query(Diretor).filter_by(nome=nome).first()
    if not diretor:
        # cria um novo objeto
        diretor = Diretor(nome=nome, nacionalidade = nacionalidade)
        session.add(diretor) #preparando-o para ser inserido no banco de dados.
        session.commit() #Confirma as alterações no banco de dados, inserindo o novo diretor.
        print(f"Diretor {nome} Adicionado!")

def excluir_diretor(id_diretor):
 #Cria uma consulta de exclusão para a tabela Diretor, filtrando pela chave primária id   
    diretor = delete(Diretor).where(Diretor.id == id_diretor)

    session.execute(diretor)  #Executa a consulta de exclusão no banco de dados.
    session.commit()
...
def excluir_filme(titulo):
    #excluir quando o valor digitado for igual ao titulo
    filme = delete(Filme).where(Filme.titulo == titulo)
    
    session.execute(filme, {'Filme': titulo}) #consulta
...
def cadastrar_cliente(nome, cpf, dataNasc, sexo):
    ...
    try:
        ...
        if idade < 18:   #levanta uma exessao
...
def fazer_locacao(nomeFilme, cpf, funcionario_id):
    ...
    try:
        ...
        if not filme or filme.qtdDisponivel <= 0:
            ...
            return
        #caso cliente nao cadastrado
        cliente = session.query(Cliente).filter_by(cpf=cpf).first()
        if not cliente:
            print("Cliente não encontrado.")
            return
         #caso funcionnario nao cadastrado
        funcionario = session.query(Funcionario).filter_by(id=funcionario_id).first()
        if not funcionario:
...
#essa linha verifica se o script está sendo executado diretamente (como um script principal) e não sendo importado como um módulo em outro script  
# Base.metadata: Referencia a metadados das classes que herdam de Base, ou seja, as definições das tabelas do banco de dados. create_all(engine): Cria todas as tabelas definidas nos metadados no banco de dados conectado ao engine. O engine é um objeto que representa a conexão com o banco de dados.       
